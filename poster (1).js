...
const poster = {
   ...
   *    Taxmin: `sum` KPI Mondo'da kuzatilgan naqshga o'xshab (dash.getTransactions
   *    summalarni TIYIN'da qaytargani) — TIYIN'da kutilishi mumkin, so'mda emas.
   *    Shuning uchun endi birlik narxi 100ga ko'paytirilib yuborilyapti.
   *    DIQQAT: bu ham hali tasdiqlanmagan taxmin — agar yordam bermasa,
   *    Poster'ning texnik yordamidan (t.me/poster_support_bot) 32-xato
   *    ma'nosini so'rash eng ishonchli keyingi qadam.
   *
  ...
  async createSupply({ businessDate, comment, items }) {
    ...
    const body = {
      ...
      ingredient: items.map((it) => ({
        ...
        sum: String(Math.round(it.unitPrice * 100)), // birlik narxi TIYIN'da (KPI Mondo tajribasidagi naqshga o'xshab)
      })),
    };
